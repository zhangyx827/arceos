...
use crate::{backend::Backend, page_iter::PAGE_SIZE_2M, backend::BackendOps};
    ...
    pub fn try_collapse_page(
        ...
        while *pages_scan < pages_to_scan && vaddr + PAGE_SIZE_2M <= area_end {
            ...
            if pte_none < max_pte_none {
                ...
                let vaddr_alloc = match global_allocator().alloc_pages(num_pages, page_size, UsageKind::UserMem) {
                    Ok(v) => VirtAddr::from(v),
                    Err(_) => continue,
                };
                unsafe { core::ptr::write_bytes(vaddr_alloc.as_mut_ptr(), 0u8, page_size); }
                let new_pa = virt_to_phys(vaddr_alloc);
                let _ = backend.collapse_page(m_start, m_end, &mut self.pt.modify(), new_pa);
